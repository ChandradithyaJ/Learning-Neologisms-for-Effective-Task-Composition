...
def analyze_base_results(json_files, output_file="base_analysis.json"):
    """
    Analyze game results from multiple JSON files and count how many times
    BASE was chosen for each image index.
    ...
    # Dictionary to track BASE choices per image
    # Key: image index (e.g., "img_00000")
    # Value: count of times BASE was chosen
    base_choices = defaultdict(int)
    ...
    for json_file in json_files:
        ...
        try:
            ...
            for choice in data.get('choices', []):
                ...
                # If BASE was chosen, increment count
                if actual_type == 'base':
                    base_choices[image_name] += 1
    ...
    results = []
    for image_name in sorted(base_choices.keys()):
        results.append({
            'image': image_name,
            'base_chosen_count': base_choices[image_name],
            'total_appearances': image_appearances[image_name],
            'base_chosen_percentage': round((base_choices[image_name] / image_appearances[image_name] * 100), 2) if image_appearances[image_name] > 0 else 0
        })
    
    # Sort by base_chosen_count (descending - most chosen first)
    results.sort(key=lambda x: x['base_chosen_count'], reverse=True)
    
    # Create output data
    output_data = {
        'total_json_files_processed': len(json_files),
        'total_images_analyzed': len(results),
        'ordered_by': 'base_chosen_count (descending)',
        'results': results
    }
    ...
    # Print top 10
    print("\nTop 10 images where BASE was chosen most:")
    for i, result in enumerate(results[:10], 1):
        print(f"  {i}. {result['image']}: BASE chosen {result['base_chosen_count']} times ({result['base_chosen_percentage']}%)")
...
if __name__ == "__main__":
    ...
    # Run the analysis
    analyze_base_results(json_files, output_file="base_analysis_ordered.json")